...
```{r Compare to Kieffer et al gene signatures}
library(data.table)
library(HGNChelper)
kieffer_2020_normal <- fread("/home/kevin/Documents/PhD/CAF_data/study_details/Costa2018/signatures.txt", select = c("Normal Fibroblast"))
kieffer_s1 <- fread("/home/kevin/Documents/PhD/CAF_data/study_details/Costa2018/signatures.txt", select = c("CAF-S1"))
kieffer_s1 <- kieffer_s1[-c(which(kieffer_s1$`CAF-S1` == "")),]$`CAF-S1`
kieffer_normal_corrected <- checkGeneSymbols(kieffer_2020_normal$`Normal Fibroblast`, species = "human")$Suggested.Symbol
kieffer_s1_corrected <- checkGeneSymbols(kieffer_s1$`CAF-S1`, species = 'human')$Suggested.Symbol
checkGeneSymbols(kieffer_s1$`CAF-S1`, species = 'human')
kieffer_s1_suggested <- checkGeneSymbols(kieffer_s1, species = 'human')$Suggested.Symbol
kieffer_s1_corrected <- kieffer_s1_suggested[!is.na(kieffer_s1_suggested)]
is.na(checkGeneSymbols(kieffer_s1, species = 'human')$Suggested.Symbol)
intersect(kieffer_normal_corrected, s3_gs)
intersect(kieffer_s1_corrected, s1_gs)
outersect(kieffer_s1_corrected, s1_gs)
```
...
### logCPM normalisation for GSVA

```{r}
library(edgeR)
y <- DGEList(counts = assay(dds_inhouse))
logCPM <- cpm(y, log = TRUE)

caf_es_cpm <- gsva(expr = logCPM,
               gset.idx.list = genesets,
               method = "gsva",
               kcdf = "Gaussian",
               mx.diff=FALSE)
...
### GSVA Kieffer et al gene signatures

```{r GSVA for Kieffer et al signatures}
kieffer_sigs <- list(S1_Kieffer = kieffer_s1_corrected, Normal_Kieffer = kieffer_normal_corrected)
kieffer_es <- gsva(expr = dds_inhouse,
