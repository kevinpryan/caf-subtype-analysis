...
```
```{r Get TPM expression for CIBERSORTx to make Signature matrix}
...
```{r Look for expression of genes used to FACS sort cells}
...
Plan: Batch correct S1, S3 and S4 if possible, do not preserve differences between CAF and TAN.
Create signature matrix for CIBERSORTx
Possibly: TPM normalise, probably optional
Run CIBERSORTx to figure out proportions of S1, S3 and S4 in In-house samples

```{r Batch correction of samples with known subtype}
