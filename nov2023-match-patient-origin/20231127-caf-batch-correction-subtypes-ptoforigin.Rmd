...
```{r}
nvars_function <- function(vsd, ntop = 500){
    rv <- rowVars(assay(vsd))
  # select the ntop genes by variance
  select <- order(rv, decreasing=TRUE)[seq_len(min(ntop, length(rv)))]
  vsd_out <- vsd
  assay(vsd_out) <- assay(vsd_out)[,select] 
  return(vsd_out)
}
```
...
remove_batch_effect <- function(vsd.mat, include_study, include_patient){
  vsd.mat.batch.corrected <- limma::removeBatchEffect(x = vsd.mat, 
                                                    batch = coldata_subpop$Study, 
                                                    batch2 = batch
                                                    )
...
nvars_function_matrix <- function(vsd.mat, ntop = 500){
    rv <- rowVars(vsd.mat)
  # select the ntop genes by variance
  select <- order(rv, decreasing=TRUE)[seq_len(min(ntop, length(rv)))]
  vsd.mat.out <- vsd.mat[select,] 
  return(vsd.mat.out)
}
vsd.mat.batch.corrected.500 <- nvars_function_matrix(vsd.mat.batch.corrected)
```

```{r}
p_batch_corrected <- pca(vsd.mat.batch.corrected.500, metadata = metadata_pca)
...
biplot(p_batch_corrected,   showLoadings = T, colby = "Subpopulation", legendPosition = "right", lab = NULL, title = "PCA biplot batch corrected by subpopulation")
biplot(p_batch_corrected,   showLoadings = T, colby = "Study", legendPosition = "right", lab = NULL, title = "PCA biplot batch corrected by study")

biplot(p,   showLoadings = T, colby = "Subpopulation", legendPosition = "right", lab = NULL, title = "PCA biplot before batch correction by subpopulation")
biplot(p,   showLoadings = T, colby = "Study", legendPosition = "right", lab = NULL, title = "PCA biplot before batch correction by study")

biplot(p,   showLoadings = T, colby = "Tumor_JuxtaTumor", legendPosition = "right", lab = NULL, title = "PCA biplot before batch correction by tumour juxtatumour")
biplot(p_batch_corrected,   showLoadings = T, colby = "Tumor_JuxtaTumor", legendPosition = "right", lab = NULL, title = "PCA biplot after batch correction by tumour juxtatumour")
...
non.log.vsd.mat.batch.corrected <- rownames_to_column(as.data.frame(non.log.vsd.mat.batch.corrected), var = "Gene")
colnames(non.log.vsd.mat.batch.corrected) <- gsub("\\.", "-", colnames(non.log.vsd.mat.batch.corrected))
stopifnot(all(dds$names == colnames(non.log.vsd.mat.batch.corrected[2:ncol(non.log.vsd.mat.batch.corrected)])))
non.log.vsd.mat.batch.corrected.for.cibersort <- non.log.vsd.mat.batch.corrected
non.log.vsd.mat.batch.corrected.for.cibersort <- rbind(c("Gene", dds$Subpopulation), non.log.vsd.mat.batch.corrected.for.cibersort)
#colnames(non.log.vsd.mat.batch.corrected.for.cibersort)[2:ncol(non.log.vsd.mat.batch.corrected.for.cibersort)] <- dds$Subpopulation
dds$Subpopulation
```

```{r}
s1_cols <- which(non.log.vsd.mat.batch.corrected.for.cibersort[1,] == "S1")
s3_cols <- which(non.log.vsd.mat.batch.corrected.for.cibersort[1,] == "S3")
s4_cols <- which(non.log.vsd.mat.batch.corrected.for.cibersort[1,] == "S4")
...
non.log.vsd.mat.batch.corrected.for.cibersort.order <- cbind.data.frame(non.log.vsd.mat.batch.corrected.for.cibersort$Gene,
                                              non.log.vsd.mat.batch.corrected.for.cibersort[,s1_cols],
                                              non.log.vsd.mat.batch.corrected.for.cibersort[,s3_cols],
                                              non.log.vsd.mat.batch.corrected.for.cibersort[,s4_cols]
                                              )
colnames(non.log.vsd.mat.batch.corrected.for.cibersort.order) <- NULL
rownames(non.log.vsd.mat.batch.corrected.for.cibersort.order) <- NULL
#cols_no_inhouse <- c("genes", rep("S1", s1_cols_length), rep("S3", s3_cols_length), rep("S4", s4_cols_length))
#non.log.vsd.mat.batch.corrected.for.cibersort.order <- rbind.data.frame(cols_no_inhouse, non.log.vsd.mat.batch.corrected.for.cibersort.order)
```

```{r}
write.table(non.log.vsd.mat.batch.corrected.for.cibersort.order, file = out, sep = "\t", quote = F, row.names = F, col.names = F)
```
