...
counts_matrix_in <- tibble::rownames_to_column(counts_matrix_in, var = "Gene")
counts_matrix_combined <- full_join(counts_matrix_in, counts_matrix_inhouse)
rownames(counts_matrix_combined) <- counts_matrix_combined$Gene 
counts_matrix_combined <- counts_matrix_combined %>% dplyr::select(-Gene)
coldata_combined <- rbind.data.frame(coldata_subpop, coldata_inhouse)
dds <- DESeqDataSetFromMatrix(countData=round(counts_matrix_combined), colData=coldata_combined, design=~ 1)
...
# set up model matrix
Tumor_JuxtaTumor <- factor(coldata_combined$Tumor_JuxtaTumor, levels=c("juxtatumor","tumor"))
design <- model.matrix(~Tumor_JuxtaTumor)

result_list <- lapply(1:nrow(binary_matrix), function(i) {
  row_vector <- as.vector(binary_matrix[i, ])
  limma_batch_effect_include_inhouse(binary_vector = row_vector, 
                     vsd = vsd.mat, 
                     #model.matrix = matrix(1,nrow(design),1), 
                     model.matrix = design,
                     coldata = coldata_combined,
                     extra_info = "combine_inhouse_subpop")
})
subpop_file <- c()
inhouse_file <- c()
library(stringr)
for (i in 1:length(result_list)){
  subpop_file <- c(subpop_file, str_split_fixed(result_list[[i]][[1]], pattern = "/", n = 9)[,9])
  inhouse_file <- c(inhouse_file, str_split_fixed(result_list[[i]][[3]], pattern = "/", n = 9)[,9])
}
outfile_subpop_inhouse <- data.frame(subpop = subpop_file, inhouse = inhouse_file)
outfile_subpop_inhouse_name <- paste("~/Documents/PhD/subtypes/caf-subtype-analysis/nov2023-match-patient-origin/outputs/limma_batch_correction/", Sys.Date(), "_filenames_limma_remove_batch_effect_batchcorrection_combined_inhouse_subpop", ".csv", sep = "")
write.csv(x = outfile_subpop_inhouse, file = outfile_subpop_inhouse_name, quote = F, col.names = F, row.names = F)
for (i in 1:length(result_list)){
  write.table(result_list[[i]][[2]], file = result_list[[i]][[1]], sep = "\t", quote = F, row.names = F, col.names = F)
  write.table(result_list[[i]][[4]], file = result_list[[i]][[3]], sep = "\t", quote = F, row.names = F, col.names = F)
}
...
vsd.mat.without.inhouse <- vsd.mat[,which(!colnames(vsd.mat) %in% coldata_inhouse$names)]
if (length(S1) != length(colnames(vsd.mat.without.inhouse))){
  stop("length s1 row incorrect")
} else if (length(S3) != length(colnames(vsd.mat.without.inhouse))) {
  stop("length s3 row incorrect")
} else if (length(S4) != length(colnames(vsd.mat.without.inhouse))) {
