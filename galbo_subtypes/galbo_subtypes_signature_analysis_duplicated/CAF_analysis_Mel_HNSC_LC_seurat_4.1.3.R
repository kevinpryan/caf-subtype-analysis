...
library(vroom)
#GSE72056 (MEL)
#gse72056_all_cells_cafs <- read.table(file.choose(), header = T, row.names = F, sep = '\t')
gse72056_all_cells_cafs <- vroom("~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/GSE72056_melanoma_single_cell_revised_v2.txt")
n_occur <- data.frame(table(gse72056_all_cells_cafs$Cell))
dups <- gse72056_all_cells_cafs[gse72056_all_cells_cafs$Cell %in% n_occur$Var1[n_occur$Freq > 1],]
not_dups <- gse72056_all_cells_cafs[!(gse72056_all_cells_cafs$Cell %in% n_occur$Var1[n_occur$Freq > 1]),]
rm(gse72056_all_cells_cafs)
dups_summarise <- dups %>% group_by(Cell) %>% dplyr::summarise((across(where(is.numeric), median)))
gse72056_all_cells_cafs_not_dup <- rbind.data.frame(not_dups, dups_summarise)
gse72056_all_cells_cafs_not_dup <- data.frame(gse72056_all_cells_cafs_not_dup)
rm(not_dups)
caf_cols <- which(gse72056_all_cells_cafs_not_dup[3,] == 5)
#gse72056_all_cells_cafs_not_dup_caf_only <- cbind(gse72056_all_cells_cafs_not_dup$Cell, gse72056_all_cells_cafs_not_dup[,caf_cols])
rownames(gse72056_all_cells_cafs_not_dup) <- gse72056_all_cells_cafs_not_dup$Cell
gse72056_all_cells_cafs_not_dup_caf_only <- data.frame(gse72056_all_cells_cafs_not_dup[4:nrow(gse72056_all_cells_cafs_not_dup),caf_cols])
rm(gse72056_all_cells_cafs_not_dup)
# added min.cells = 3 in order to get same number of genes as authors
GSE72056_melanoma_cafs <- CreateSeuratObject(counts = gse72056_all_cells_cafs_not_dup_caf_only, project = "GSE72056_CAF_Melanoma", min.cells = 3)
...
GSE72056_melanoma_cafs_resolution_0.5.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
GSE72056_melanoma_cafs_resolution_0.5.markers_top20 <- GSE72056_melanoma_cafs_resolution_0.5.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
...
saveRDS(GSE72056_melanoma_cafs_resolution_0.5, file = "~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/outputs/GSE72056_melanoma_cafs_resolution_0.5_seurat_4.1.3.Rds")

#GSE103322(HNSC)
gse103322_cafs <- read.table("~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/GSE103322_HNSCC_all_data.txt", header = T, row.names = 1, sep = '\t')
caf_cols_hnsc <- which(gse103322_cafs[5,] == "Fibroblast")
gse103322_all_cells_cafs_caf_only <- as.data.frame(gse103322_cafs[6:nrow(gse103322_cafs),caf_cols_hnsc])
rm(gse103322_cafs)
gse103322_all_cells_cafs_caf_only <- mutate_all(gse103322_all_cells_cafs_caf_only, function(x) as.numeric(as.character(x)))
## TODO should I add a min.cells = 3 here too?
GSE103322_hnsc_cafs <- CreateSeuratObject(counts = gse103322_all_cells_cafs_caf_only, project = "GSE103322_CAF_HNSC", min.cells = 3)
...
GSE103322_hnsc_cafs_resolution_0.3.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
GSE103322_hnsc_cafs_resolution_0.3.markers_top20 <- GSE103322_hnsc_cafs_resolution_0.3.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
...
hnscc_dot_plot <- DotPlot(GSE103322_hnsc_cafs_resolution_0.3, features = feature, cols = c("lightgrey", "red")) + RotatedAxis()
saveRDS(GSE103322_hnsc_cafs_resolution_0.3, file = "~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/outputs/GSE103322_hnsc_cafs_resolution_0.3_hnsc_seurat_4.1.3.Rds")
#E-MTB_6149 (LC)
infile <- "~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/E-MTAB-6149/Fibro.Cellview.Rds"
LC_cafs2 <- load(infile)
featuredata_in_log2cpm <- featuredata[rownames(log2cpm),]
stopifnot(all.equal(rownames(log2cpm), rownames(featuredata_in_log2cpm)))
rownames(log2cpm) <- featuredata_in_log2cpm$Associated.Gene.Name
LC_cafs <- CreateSeuratObject(counts = log2cpm, project = "CAF_LC", min.cells = 3)
dim(LC_cafs)
...
LC_cafs.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
LC_cafs.markers_top20 <- LC_cafs.markers %>% dplyr::group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
...
saveRDS(LC_cafs, file = "~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/outputs/LC_cafs_seurat_4.1.3.Rds")

## tweaking resolution until I get the same number of clusters as the original authors
LC_cafs_2 <- FindClusters(LC_cafs, resolution = 0.05)
LC_cafs_2 <- RunUMAP(LC_cafs_2, dims = 1:20)
DimPlot(LC_cafs_2,reduction = "umap")

LC_cafs.markers2 <- FindAllMarkers(LC_cafs_2, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
LC_cafs.markers2 %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
LC_cafs.markers_top20_2 <- LC_cafs.markers2 %>% dplyr::group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
DoHeatmap(LC_cafs_2, features = LC_cafs.markers_top20_2$gene, label = F) + NoLegend() + scale_fill_gradientn(colors = c("blue", "white", "red"), na.value = "white")
feature <- c('COL1A1', 'COL1A2', 'COL3A1', 'COL8A1', 'ACTA2', 'MYH11', 'CFD', 'IL6', 'CCL2', 'CXCL2', 'EEF1A1', 'RPL17')
LC_dot_plot2 <- DotPlot(LC_cafs, features = feature, cols = c("lightgrey", "red")) + RotatedAxis()
saveRDS(LC_cafs_2, file = "~/Documents/PhD/subtypes/caf-subtype-analysis/galbo_subtypes/data/outputs/LC_cafs_2_seurat_4.1.3.Rds")
